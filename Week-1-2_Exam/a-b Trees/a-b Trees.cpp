...
using namespace std;
 const int MAX = 4 ;
 int MIN = 2 ;//default;replaced with a b4 construction
struct btnode
{
    int count ;
    int value[MAX + 1] ;
...
void btree :: display ( btnode *root )
{
    ...
    if ( root != NULL )
    {
        ...
        {
            cout << " for child " << (i+1) << " of " << root->count << endl;
            cout << "head" << (i+1) << " : " <<root -> value[i + 1] << "," ;
            //cout << root -> value[i + 1] << "," ;
...
//cout << a << " : " << b << endl;
if (a>b || 2*a > b+1 || a<1){
    cout << "(a-b) tree construction is not possible";
    return 0;
}    
MIN = a;
//MAX = b;    
